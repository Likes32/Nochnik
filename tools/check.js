...
console.log("\nHTML — смысловые блоки не спрятаны");
{
  ...
  const ALLOWED = [
    ...
    /\.nav__list \.mono/,        // факты у пунктов меню: нужны в полноэкранной
                                 // панели, в строку десктопной шапки не влезут
    /\.nav__price/,              // цена в меню дублирует ту, что на десктопе
                                 // и так видна на первом экране и в брони
    /\.sheet__/,                 // язычок, крестик и затемнение шторки:
                                 // на десктопе меню — строка, шторки нет
  ];
