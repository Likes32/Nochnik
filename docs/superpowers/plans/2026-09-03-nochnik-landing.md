...
- [x] **Шаг 6: `assets/logo-primary.svg` и `assets/logo-compact.svg`**

Код обоих знаков — в спеке, раздел 2. Скопировать дословно.

- [x] **Шаг 7: `index.html` — скелет**
...
- [x] **Шаг 1: разметка секции в `index.html`**
...
- [x] **Шаг 2: первый экран, метка 17:40**
...
- [x] **Шаг 3: остальные шесть секций**
...
- [x] **Шаг 2: подписи площадей прямо на схеме**
...
- [x] **Шаг 5: проверить на трёх ширинах**
...
- [x] **Шаг 5: `js/reveal.js` и режим `.lite`**
...
- [x] **Шаг 4: запушить и проверить живой адрес**
...
Все восемь задач выполнены, сайт живёт на https://likes32.github.io/Nochnik/

**Отступления от плана, все осознанные:**

1. **Шрифты не в data-URI, а самохостингом.** data-URI был нужен «Косе»
   ради работы с file://, здесь проверка идёт через локальный сервер.
   Google отдаёт вариативные файлы — начертания 300 и 500 физически
   один файл, поэтому качаем по одному на семейство и диапазон:
   6 файлов и 132 КБ вместо 10 и 264.

2. **GSAP не вендорился.** Он нужен был ради одного параллакса первого
   экрана — это пятнадцать строк в том же rAF-обработчике, который
   и так считает прогресс ночи. Lenis оставлен: lerp 0.14, только десктоп.

3. **Метки времени убраны** по просьбе заказчика уже после сборки;
   вместе с ними ушла рельса левого поля.

4. **Схемы вместо двух стали тремя** — добавлен план участка.

5. **Штриховка занятых дат в календаре** — оговорённое исключение
   из запрета на градиенты: она кодирует состояние вторым признаком
   помимо цвета.
...
CSS — прозрачные SVG-зоны ловят указатель
  ✓ проверено правил с прозрачной заливкой: 17

HTML — смысловые блоки не спрятаны
  ✓ display:none только там, где положено

HTML — интерактивные зоны доступны с клавиатуры
  ✓ интерактивных зон: 18, все с tabindex и role

Всё чисто. — статические проверки: баланс скобок в CSS,
  прозрачные SVG-зоны без pointer-events, display:none вне списка
  разрешённых, клавиатурная доступность зон.
- Использование: node tools/photos.js find | get <slot> <id> — подбор и загрузка кадров через Pexels.
-  — просмотр кадрирования сцен без прокрутки.
-  — локальный сервер без зависимостей.
-   cormorant-latin.woff2  36.8 КБ  начертания 300 500
  cormorant-cyrillic.woff2  20.7 КБ  начертания 300 500
  onest-latin.woff2  33.0 КБ  начертания 400 600
  onest-cyrillic.woff2  15.5 КБ  начертания 400 600
  mono-latin.woff2  20.7 КБ  начертания 400
  mono-cyrillic.woff2  5.2 КБ  начертания 400

Скачано файлов: 6. fonts.css собран. — сборка fonts.css с дедупликацией.
